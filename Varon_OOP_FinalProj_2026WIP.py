...
class DNA(Seq):
    ...
    def six_frames(self):
        ...
        for n in range(len(reverse_sequence)-2):
            codon = reverse_sequence[n:n+3]
            frames.append(codon)
        return(frames)
    ...
    def start_codon(self):
        ...
        for n in range(0,len(self.sequence)-2):
           if self.sequence[n:n+3] == "AUG":
             metcode=self.sequence[n:len(self.sequence)]
        ...
        for i in range(0,len(reverse_sequence)-2):
            if reverse_sequence[i:i+3] == "AUG":
                metcode=reverse_sequence[i:len(reverse_sequence)]
                self.met_frames.append(metcode)
...
class Protein(Seq):
    def __init__(self,sequence,gene,species,geneid,kmers=[],**kwargs):
        super().__init__(sequence,gene,species,kmers)
        messy_sequence=sequence.upper().replace("T","U").strip()
...
if __name__ == "__main__":
    testRna=RNA("AGGAUGCGUAUGAGG","TsGen","elliot","sillyGene")
    testRna.start_codon()
    print(testRna.met_frames)
    
    '''
    otherRna=RNA("AUGGAGGCGAUGAGG","TsGen2","elliot","sillyGene")
    print(testRna+otherRna)
'''
