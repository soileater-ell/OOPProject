...
#  (2) Add total_hydro, which return the sum of the totalbicity of a self.sequence
...
class DNA(Seq):
    ...
    def __init__(self,sequence,gene,species,geneid,kmers=[],frames=[],**kwargs):
        super().__init__(sequence,gene,species,kmers)
        self.sequence=re.sub('[^ATGCU]','N',sequence.upper().strip())
        self.geneid=geneid
        self.frames=frames
    ...
    def six_frames(self):
        ...
        >>> testDNA=DNA('AATACATGATG', 'TESTgene', "hum", "3")
        >>> print(testDNA)
        AATACATGATG
        >>> rf = testDNA.six_frames()
        >>> print(rf)
        ['AATACATGATG', 'ATACATGATG', 'TACATGATG', 'CATCATGTATT', 'ATCATGTATT', 'TCATGTATT']
        ...
        self.frames=[]
        for n in range(3):
            frame = self.sequence[n:len(self.sequence)]
            self.frames.append(frame)
        compseq=self.sequence.replace("A","t").replace("T","a").replace("C","g").replace("G","c")
        reverse_sequence=compseq.upper()[::-1]
        for n in range(3):
            frame = reverse_sequence[n:len(reverse_sequence)]
            self.frames.append(frame)
        return(self.frames)
...
class RNA(DNA):
    def __init__(self,sequence,gene,species,geneid,kmers=[],codons=[],met_frames=[],**kwargs):
        super().__init__(sequence,gene,species,geneid,kmers,**kwargs)
        self.sequence=sequence.upper().replace("T","U").strip()
        self.gene=gene
        self.species=species
        self.geneid=geneid
        self.kmers=kmers
        self.codons=codons 
        self.met_frames=met_frames
    ...
    def translate(self):
        return[standard_code.get(codon) for codon in self.codons]
...
class Protein(Seq):
    def __init__(self,sequence,gene,species,geneid,kmers=[],**kwargs):
        super().__init__(sequence,gene,species,kmers)
        messy_sequence=sequence.upper().strip()
        self.sequence=re.sub("[^A-Z]", "X",messy_sequence)
        self.gene=gene
        self.species=species
        self.geneid=geneid
        self.kmers=kmers
    ...
    def total_hydro(self): 
        '''
	returns sum of hydrophobicity of all amino acids in a protein sequence
	>>> testProt=Protein("MAGSDYFLKT", "tsProt", 'hum', 'ts3')    
	>>> th=testProt.total_hydro()
    >>> print(th)
    -0.3
